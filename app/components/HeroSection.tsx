...
export default function HeroSection() {
  ...
  const linkedIn = profile.socials.find((social) => social.label === "LinkedIn")?.href ?? "https://www.linkedin.com/in/bobbyanhp/";
  const gitHub = profile.socials.find((social) => social.label === "GitHub")?.href ?? "https://github.com/bobbyanhp";
    ...
    <div className="relative overflow-hidden rounded-[2.5rem] bg-[#eaf5ff] shadow-sm">
      <section id="home" className="relative grid gap-8 px-4 pb-10 pt-2 sm:px-6 md:grid-cols-2 md:gap-0 md:px-10 md:pb-16">
        <div className="z-10 max-w-xl py-6 sm:py-10">
          ...
          <h1 className="mt-2 text-[clamp(2rem,5vw,3.5rem)] font-extrabold leading-tight text-indigo-800">{profile.name}</h1>
          <p className="mt-4 max-w-prose text-[clamp(1.35rem,2.8vw,2.1rem)] font-semibold leading-tight text-slate-800 sm:mt-6">{profile.intro}</p>

          <div className="mt-7 flex max-w-3xl flex-wrap gap-3 sm:gap-4">
            {heroSkills.map((item) => (
              <span key={item.id} className="inline-flex items-center gap-3 rounded-full border-2 border-indigo-200/90 bg-white/45 px-5 py-3 text-sm font-semibold text-slate-700 shadow-sm backdrop-blur sm:text-base">
...
function SocialButton({ label, href, children }: { label: string; href: string; children: ReactNode }) {
  return (
    <a aria-label={label} href={href} target="_blank" rel="noopener noreferrer" className="grid h-8 w-8 place-items-center rounded-full bg-[#eaf5ff] text-slate-700 hover:ring-2 hover:ring-indigo-200" title={label}>
      {children}
    </a>
  );
}
